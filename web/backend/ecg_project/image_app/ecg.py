...
class ECG:
    ...
    def SignalExtraction_Scaling(self, Leads):
        if Leads is None:
            raise ValueError("Leads should not be None")
    ...
    def DimensionalReduciton(self, test_final):
        pca_loaded_model = joblib.load(os.path.join(os.getcwd(), 'image_app', 'models', 'PCA_ECG (1).pkl'))
    ...
    def ModelLoad_predict(self, final_df):
        # Load the model
        loaded_model = load(os.path.join(os.getcwd(), 'image_app', 'models', 'Heart_Disease_Prediction_using_image_processing__ECG.pkl'))
