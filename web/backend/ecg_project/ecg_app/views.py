...
def plot_ecg_with_annotations(X_df, age, gender, superclass_labels, class_names, filename):
    ...
    for i in range(X_df.shape[1]):
        sns.lineplot(x=np.arange(X_df.shape[0]), y=X_df.iloc[:, i], ax=axes[i], color='black')
        axes[i].grid(True)
...
def predict_ecg(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            # Load raw ECG data
            record = wfdb.rdrecord(dat_path.split('.')[0])  # .dat file without extension
            X = record.p_signal  # ECG data

            # Prepare DataFrame
            X_df = pd.DataFrame(X, columns=[f'channel-{i+1}' for i in range(X.shape[1])])

            # Load preprocessor
            data_preprocessor_dict = joblib.load(r'D:\Semester 7\django\ecg_project\data_preprocessor.pkl')
            ...
            model = ECGClassifier(X_df.shape[1], 128, per_cat_nunique, 30, 1, 128, 5)
            model.load_state_dict(torch.load(r'D:\Semester 7\django\ecg_project\pytorch_ecg_rnn.pth', map_location='cpu'))
            model.eval()
            ...
            # Plot ECG
            plot_filename = os.path.join(settings.MEDIA_ROOT, 'uploads', 'ecg_image.png')
            plot_ecg_with_annotations(X_df, age, gender, superclass_labels, class_names, plot_filename)
